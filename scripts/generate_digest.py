...
"""
PresidencyClock — авто-дайджести (мультимовний + hreflang) для структури:
<repo>/
  potus-countdown/
    scripts/generate_digest.py   ← цей файл
    ... (тут лежить сайт)
Вивід іде в папку `potus-countdown/` (корінь сайту).

ENV:
- LANGS=uk,de,fr,es
- TIMEZONE=Europe/Kyiv
- DIGEST_MIN_ITEMS=8
- OPENAI_API_KEY=<ключ>  (щоб тексти були мовою сторінки)
...
<head>
  ...
  <meta name="description" content="{{ meta_desc }}"><meta name="google-adsense-account" content="ca-pub-5023415479696725">
...
<body>
  <div class="wrap">
    ...
    <div class="latest">
      ...
      <div class="grid">
        {% for it in preview %}
        <div class="card"><h3>{{ it.headline }}</h3><p>{{ it.summary }}</p><div><a href="{{ it.url }}" target="_blank" rel="nofollow noopener">{{ readmore }}</a> — {{ it.source }}</div></div>
        {% endfor %}
      </div>
    ...
    <div class="dates">
      <h2>{{ archive }}</h2>
      <ul>{% for d in dates %}<li><a href="/{{ lang }}/digest/{{ d }}/">{{ d }}</a></li>{% endfor %}</ul>
    </div>
  </div>
</body>
</html>
...
def summarize_smart(lang, items):
  ...
  res=_openai_chat([{"role":"system","content":sys},{"role":"user","content":json.dumps({"items":compact})}], model=OPENAI_MODEL, temperature=0.2, max_tokens=2000)
...
def render_index(lang):
  ...
  if latest_date:
    ...
    if jpath.exists():
      try: preview=json.loads(jpath.read_text(encoding="utf-8"))[:3]
...
def main():
  ...
  raw=fetch_all(); items=score_and_pick(dedupe(raw), limit=24)
  if len(items) < MIN_ITEMS:
    log(f"Below threshold ({len(items)}<{MIN_ITEMS}) — skip."); return
  if llm_available():
  ...
  for lang in LANGS:
    ...
    render_page(lang, date_str, multi[lang]); render_index(lang)
