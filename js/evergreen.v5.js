(function(){
  console.log('evergreen.v5 loaded', (document.currentScript && document.currentScript.src) || 'inline');

  const $ = s => document.querySelector(s);
  const $$ = s => [...document.querySelectorAll(s)];

  // мова
  const params = new URLSearchParams(location.search);
  let lang = (params.get('lang') || localStorage.getItem('lang') || (navigator.language||'en'))
              .slice(0,2).toLowerCase();
  if (!I18N[lang]) lang = 'en';
  const t = k => I18N[lang][k] || k;

  // список країн
  const countries = ['us','ua','ca','mx','es','fr','de'];
  let active = countries[0];

  function applyI18n(){
    $$('[data-i18n]').forEach(el => el.textContent = t(el.dataset.i18n));
    const langBox = $('#lang'); langBox.innerHTML = '';
    ['en','uk','es','fr','de'].forEach(code=>{
      const b = document.createElement('button');
      b.className = 'btn'; b.textContent = code.toUpperCase();
      b.onclick = () => { lang = code; localStorage.setItem('lang', lang); applyI18n(); render(active); };
      langBox.appendChild(b);
    });
  }

  function buildTabs(){
    const tabs = $('#tabs'); tabs.innerHTML = '';
    countries.forEach(code=>{
      const b = document.createElement('button');
      b.className = 'btn';
      b.textContent = (I18N[lang].country && I18N[lang].country[code]) ? I18N[lang].country[code] : code.toUpperCase();
      b.setAttribute('aria-pressed', String(code===active));
      b.onclick = () => { active = code; render(code); };
      tabs.appendChild(b);
    });
  }

  async function load(code){
    try{
      const res = await fetch(`/assets/evergreen/${code}.json?cb=${Date.now()}`);
      if (!res.ok) return [];
      const arr = await res.json();

      const parseDate = (s) => {
        if (!s || /^(present|incumbent|now|current)$/i.test(String(s).trim())) return new Date();
        const d = new Date(s);
        return isNaN(d) ? null : d;
      };

      return arr.map(x => {
        const start = parseDate(x.start);
        const end   = parseDate(x.end) || new Date();
        const days  = start ? Math.round((end - start) / 86400000) : 0;
        return { ...x, days };
      }).sort((a,b) => a.days - b.days);

    }catch(e){ return []; }
  }

  async function render(code){
    buildTabs();
    const data = await load(code);
    const wrap = $('#table-wrap'), note = $('#note');

    if (!data.length){
      wrap.innerHTML = `<p>${t('noData') || 'Даних поки що немає.'}</p>`;
      note.textContent = `/assets/evergreen/${code}.json`;
      return;
    }

    const head = `<tr>
      <th>${(t('col')&&t('col').name)||'Name'}</th>
      <th>${(t('col')&&t('col').office)||'Office'}</th>
      <th>${(t('col')&&t('col').start)||'Start'}</th>
      <th>${(t('col')&&t('col').end)||'End'}</th>
      <th>${(t('col')&&t('col').days)||'Days'}</th>
    </tr>`;

    const rows = data.map(r => `<tr>
      <td>${r.name}</td>
      <td>${r.office || ''}</td>
      <td>${r.start}</td>
      <td>${r.end}</td>
      <td>${r.days}</td>
    </tr>`).join('');

    wrap.innerHTML = `<table>${head}${rows}</table>`;
    note.textContent = '';
  }

  document.addEventListener('DOMContentLoaded', ()=>{
    applyI18n(); buildTabs(); render(active);
  });
})();
